...
// mutex for synchronisation pthread_mutex_t condlock;
...
pthread_cond_init(&canread, NULL); pthread_cond_init(&canwrite, NULL); pthread_mutex_init(&condlock, NULL);
}

// mutex provide synchronisation so that no other thread
// can change the value of data void beginread(int i)
{
...
// reader suspended pthread_cond_wait(&canread, &condlock); waitr--;
}

// else reader reads the resource rcnt++;
cout << "reader " << i << " is reading\n"; pthread_mutex_unlock(&condlock); pthread_cond_broadcast(&canread);
}
...
// if there are no readers left then writer enters monitor pthread_mutex_lock(&condlock);
...
// or waiting readers or other writer if (wcnt == 1 || rcnt > 0) {
++waitw;
...
void endwrite(int i)
{
pthread_mutex_lock(&condlock); wcnt = 0;
// if any readers are waiting, threads are unblocked if (waitr > 0)
pthread_cond_signal(&canread); else
...
usleep(1); M.beginread(i); M.endread(i); c++;
}
}
...
int main()
{
pthread_t r[5], w[5]; int id[5];
for (int i = 0; i < 5; i++) { id[i] = i;

// creating threads which execute reader function pthread_create(&r[i], NULL, &reader, &id[i]);
